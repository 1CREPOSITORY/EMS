...
// Если в источнике у счета выключен вид учета, свойство проводки не выгружается (NULL).
//
&После("ПолучитьЗначениеСвойства")
Процедура ЦТ_ПолучитьЗначениеСвойстваПосле(Значение, ОбъектКоллекции, ПКО, ПКС, ВходящиеДанные, Источник, ВыборкаДанных)

	Если Значение = Неопределено Или Значение = NULL Тогда
		Возврат;
	КонецЕсли;

	ИмяСвойства = СокрЛП(Строка(ПКС.Источник));
	Если ПустаяСтрока(ИмяСвойства) Тогда
		ИмяСвойства = СокрЛП(Строка(ПКС.Приемник));
	КонецЕсли;

	ЗаписьПроводки = ОбъектКоллекции;
	Если ЗаписьПроводки = Неопределено Тогда
		ЗаписьПроводки = Источник;
	КонецЕсли;

	ЦТ_ОбнулитьВыгружаемоеЗначениеПоВидуУчета(Значение, ЗаписьПроводки, ИмяСвойства);

КонецПроцедуры

Процедура ЦТ_ОбнулитьВыгружаемоеЗначениеПоВидуУчета(Значение, ЗаписьПроводки, ИмяСвойства)

	Если ЗаписьПроводки = Неопределено Или ПустаяСтрока(ИмяСвойства) Тогда
		Возврат;
	КонецЕсли;

	Сторона = ЦТ_СторонаПроводкиПоИмениСвойства(ИмяСвойства);
	Если ПустаяСтрока(Сторона) Тогда
		Возврат;
	КонецЕсли;

	ИмяПризнака = ЦТ_ПризнакУчетаПоИмениСвойства(ИмяСвойства);
	Если ПустаяСтрока(ИмяПризнака) Тогда
		Возврат;
	КонецЕсли;

	СчетПроводки = ЦТ_СчетСтороныПроводки(ЗаписьПроводки, Сторона);
	Если Не ЗначениеЗаполнено(СчетПроводки) Тогда
		Возврат;
	КонецЕсли;

	Если ЦТ_ПризнакУчетаСчетаВыключен(СчетПроводки, ИмяПризнака) Тогда
		Значение = Неопределено;
	КонецЕсли;

КонецПроцедуры

Функция ЦТ_СторонаПроводкиПоИмениСвойства(ИмяСвойства)

	Если СтрЗаканчиваетсяНа(ИмяСвойства, "Дт") Тогда
		Возврат "Дт";
	ИначеЕсли СтрЗаканчиваетсяНа(ИмяСвойства, "Кт") Тогда
		Возврат "Кт";
	КонецЕсли;
...
Функция ЦТ_ПризнакУчетаПоИмениСвойства(ИмяСвойства)

	Если СтрНачинаетсяС(ИмяСвойства, "Подразделение") Тогда
		Возврат "УчетПоПодразделениям";
	ИначеЕсли СтрНачинаетсяС(ИмяСвойства, "Валюта") Или СтрНачинаетсяС(ИмяСвойства, "ВалютнаяСумма") Тогда
		Возврат "Валютный";
	ИначеЕсли СтрНачинаетсяС(ИмяСвойства, "Количество") Тогда
		Возврат "Количественный";
	КонецЕсли;
...
Функция ЦТ_СчетСтороныПроводки(ЗаписьПроводки, Сторона)

	ИмяСчета = "Счет" + Сторона;
	Попытка
		Возврат ЗаписьПроводки[ИмяСчета];
	Исключение
...
// Ищет замену субконто только на своей стороне проводки (Дт или Кт).
// Типовой метод без этого фильтра при одинаковых счетах копирует СубконтоКт в СубконтоДт.
//
...
Процедура ЦТ_УстранитьОшибкуНазначенияВидаСубконто(ЗагруженныеДвижения)
	...
	Для Каждого СтрокаТаблицы Из ЗагруженныеДвижения Цикл
		...
		ЦТ_ПроверитьКоллекциюСубконто(СтрокаТаблицы, ЗначенияДоИсправления, "Дт", ЕстьОшибка);
		ЦТ_ПроверитьКоллекциюСубконто(СтрокаТаблицы, ЗначенияДоИсправления, "Кт", ЕстьОшибка);
...
// Сверяет вид субконто в колонке таблицы с порядком видов на счете.
// При расхождении запоминает исходное значение и ставит вид из счета.
//
Процедура ЦТ_ПроверитьКоллекциюСубконто(СтрокаТаблицы, ЗначенияДоИсправления, Направление, ЕстьОшибка)

	СчетНаправления = СтрокаТаблицы["Счет" + Направление];
	Если НЕ ЗначениеЗаполнено(СчетНаправления) Тогда
		Возврат;
	КонецЕсли;

	Для Каждого ВидСубконтоИзКоллекции Из СчетНаправления.ВидыСубконто Цикл

		КлючСубконто = Направление + Строка(ВидСубконтоИзКоллекции.НомерСтроки);
		ИмяСубконто = "Субконто" + КлючСубконто;
		ИмяВидаСубконто = "ВидСубконто" + КлючСубконто;
		...
		ОписаниеСубконто.Вставить("ВидСубконто", СтрокаТаблицы[ИмяВидаСубконто]);

		Если СтрокаТаблицы[ИмяВидаСубконто] <> ВидСубконтоИзКоллекции.ВидСубконто Тогда

			ОписаниеСубконто.ЕстьОшибка = Истина;
			СтрокаТаблицы[ИмяСубконто] = Неопределено;
			СтрокаТаблицы[ИмяВидаСубконто] = ВидСубконтоИзКоллекции.ВидСубконто;
			ЕстьОшибка = Истина;

		КонецЕсли;

		ЗначенияДоИсправления.Вставить(КлючСубконто, ОписаниеСубконто);
...
Процедура Z_ЗаписатьРегистрыБухгалтерии(Регистратор, ТаблицыОтложеннойЗаписиДвижений)
	...
	Для Каждого ОписаниеТаблицыДвижений Из ТаблицыОтложеннойЗаписиДвижений Цикл
		...
		Для Каждого СтрокаТаблицы Из ТаблицаДвижений Цикл
			...
			ЦТ_ЗаменитьНулевойИдентификаторПодразделенияНаПустуюСсылку(ЗаписьНабора);
			ЦТ_ОчиститьРеквизитыПоВидамУчетаСчета(ЗаписьНабора);
			ЦТ_ЗаполнитьСубконтоЗаписиРегистра(ЗаписьНабора, СтрокаТаблицы);
...
// Нулевой UUID пустой ссылки после ПолучитьСсылку становится "Объект не найден".
// В проводках такое подразделение заменяем на настоящую пустую ссылку.
//
Процедура ЦТ_ЗаменитьНулевойИдентификаторПодразделенияНаПустуюСсылку(ЗаписьНабора)

	РеквизитыПодразделений = Новый Структура("ПодразделениеДт, ПодразделениеКт, Подразделение");
	ЗаполнитьЗначенияСвойств(РеквизитыПодразделений, ЗаписьНабора);
	...
	ТипПодразделения = Тип("СправочникСсылка.ПодразделенияОрганизаций");

	Для Каждого КлючИЗначение Из РеквизитыПодразделений Цикл

		ЗначениеПоля = КлючИЗначение.Значение;
		Если ТипЗнч(ЗначениеПоля) <> ТипПодразделения Тогда
			Продолжить;
		КонецЕсли;

		Если ЗначениеПоля.УникальныйИдентификатор() = ПустойИдентификатор Тогда
			ЗаписьНабора[КлючИЗначение.Ключ] = ПустоеПодразделение;
		КонецЕсли;
...
// Если на счете выключен вид учета, связанные реквизиты проводки не переносятся.
//
Процедура ЦТ_ОчиститьРеквизитыПоВидамУчетаСчета(ЗаписьНабора)

	ЦТ_ОчиститьСторонуПроводкиПоВидамУчета(ЗаписьНабора, "Дт");
	ЦТ_ОчиститьСторонуПроводкиПоВидамУчета(ЗаписьНабора, "Кт");

КонецПроцедуры

Процедура ЦТ_ОчиститьСторонуПроводкиПоВидамУчета(ЗаписьНабора, Сторона)

	ИмяСчета = "Счет" + Сторона;
	СчетПроводки = Неопределено;
	Попытка
		СчетПроводки = ЗаписьНабора[ИмяСчета];
	Исключение
		Возврат;
	КонецПопытки;
	...
	Если ЦТ_ПризнакУчетаСчетаВыключен(СчетПроводки, "УчетПоПодразделениям") Тогда
		ЦТ_ОчиститьРеквизитЗаписи(ЗаписьНабора, "Подразделение" + Сторона);
	КонецЕсли;

	Если ЦТ_ПризнакУчетаСчетаВыключен(СчетПроводки, "Валютный") Тогда
		ЦТ_ОчиститьРеквизитЗаписи(ЗаписьНабора, "Валюта" + Сторона);
		ЦТ_ОчиститьРеквизитЗаписи(ЗаписьНабора, "ВалютнаяСумма" + Сторона);
	КонецЕсли;

	Если ЦТ_ПризнакУчетаСчетаВыключен(СчетПроводки, "Количественный") Тогда
		ЦТ_ОчиститьРеквизитЗаписи(ЗаписьНабора, "Количество" + Сторона);
	КонецЕсли;

КонецПроцедуры

Функция ЦТ_ПризнакУчетаСчетаВыключен(СчетПроводки, ИмяПризнака)

	Попытка
		Возврат Не СчетПроводки[ИмяПризнака];
	Исключение
		Возврат Ложь;
	КонецПопытки;
...
Процедура ЦТ_ОчиститьРеквизитЗаписи(ЗаписьНабора, ИмяРеквизита)

	Попытка
		ЗаписьНабора[ИмяРеквизита] = Неопределено;
	Исключение
		Возврат;
	КонецПопытки;
...
// Заполняет субконто записи регистра бухгалтерии из строки выгруженной таблицы.
// Значения сначала запоминаются, затем кредит записывается раньше дебета.
// Иначе при одинаковых счетах платформа затирает субконто дебета значением кредита.
//
Процедура ЦТ_ЗаполнитьСубконтоЗаписиРегистра(ЗаписьНабора, СтрокаТаблицы)

	ЗначенияСубконтоДт = ЦТ_ЗначенияСубконтоИзСтрокиТаблицы(ЗаписьНабора.СчетДт, СтрокаТаблицы, "Дт");
	ЗначенияСубконтоКт = ЦТ_ЗначенияСубконтоИзСтрокиТаблицы(ЗаписьНабора.СчетКт, СтрокаТаблицы, "Кт");

	Для Каждого ОписаниеСубконтоКт Из ЗначенияСубконтоКт Цикл
		ЗаписьНабора.СубконтоКт.Вставить(ОписаниеСубконтоКт.Ключ, ОписаниеСубконтоКт.Значение);
	КонецЦикла;

	Для Каждого ОписаниеСубконтоДт Из ЗначенияСубконтоДт Цикл
		ЗаписьНабора.СубконтоДт.Вставить(ОписаниеСубконтоДт.Ключ, ОписаниеСубконтоДт.Значение);
	КонецЦикла;

КонецПроцедуры

// Собирает значения субконто из колонок СубконтоДтN / СубконтоКтN по видам счета.
//
Функция ЦТ_ЗначенияСубконтоИзСтрокиТаблицы(Счет, СтрокаТаблицы, Направление)
	...
	НомерСубконто = 0;
	Для Каждого ОписаниеВидаСубконто Из Счет.ВидыСубконто Цикл
		НомерСубконто = НомерСубконто + 1;
		ИмяКолонки = "Субконто" + Направление + Строка(НомерСубконто);
		ЗначенияСубконто.Вставить(ОписаниеВидаСубконто.ВидСубконто, СтрокаТаблицы[ИмяКолонки]);
	КонецЦикла;
